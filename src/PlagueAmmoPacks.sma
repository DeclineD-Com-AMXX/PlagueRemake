...
new const   szVaultName[] = "currency_ammo_save";
new         vault;
...
{
    fwdChangedAmmo = CreateMultiForward("Plague_SetUserAmmoPacks", ET_CONTINUE, FP_CELL, FP_CELL);
    fwdChangedAmmoPost = CreateMultiForward("Plague_SetUserAmmoPacks_Post", ET_IGNORE, FP_CELL, FP_CELL);
}

public plugin_natives()
{
    register_native("pr_set_user_ammo_packs", "Native_SetPacks");
    register_native("pr_get_user_ammo_packs", "Native_GetPacks");
}
...
public plugin_init()
{
    register_plugin("[Plague] Currency: Model (Ammo)", "v1.0", "DeclineD");

    register_concmd("amx_ammo_packs", "cmdAmmo", ADMIN_PASSWORD, "Usage: amx_ammo_packs <name/all> <set/add/take> <amount>");
    ...
    cOn = register_cvar("pr_currency_ammo_on", "1");
    cSave = register_cvar("pr_currency_ammo_save", "1");
    cSaveType = register_cvar("pr_currency_ammo_save_type", "0");
    cSaveCount = register_cvar("pr_currency_ammo_save_max", "20000");
    cSaveCountVip = register_cvar("pr_currency_ammo_save_max_vip", "50000");
    cKillReward = register_cvar("pr_currency_ammo_kill_reward", "5");
    cKillRewardVip = register_cvar("pr_currency_ammo_kill_reward_vip", "5");
    cInfectReward = register_cvar("pr_currency_ammo_infect_reward", "3");
    cInfectRewardVip = register_cvar("pr_currency_ammo_infect_reward_vip", "3");
    cKillCount = register_cvar("pr_currency_ammo_kill_to_reward", "1");
    cKillCountVip = register_cvar("pr_currency_ammo_kill_to_reward_vip", "1");
    cInfectCount = register_cvar("pr_currency_ammo_infect_to_reward", "1");
    cInfectCountVip = register_cvar("pr_currency_ammo_infect_to_reward_vip", "1");
    cDamageReward = register_cvar("pr_currency_ammo_damage_reward", "2");
    cDamageRewardVip = register_cvar("pr_currency_ammo_damage_reward_vip", "2");
    cDamageCount = register_cvar("pr_currency_ammo_damage_to_reward", "2.0");
    cDamageCountVip = register_cvar("pr_currency_ammo_damage_to_reward_vip", "2.0");
...
public cmdAmmo(id, level, cid)
{
    ...
    client_print_color(id, 0, "^4[ZP] ^3Admin ^4%n ^1%s ^4%s ^3%i ^4Ammo Packs^1.", id, (iMethod == 0 ? "set" : (iMethod == 1 ? "gave" : "took")), (player > 0 ? fmt("%n", player) : "Everyone"), iAmount);
...
{
    if(id == attacker || pr_is_human(id) || pr_is_zombie(attacker) || flDamage <= 0.0 || !(dmgBits & DMG_BULLET))
        return;
    ...
    flDamage += gData[attacker][Damage];

    while(flDamage - count > 0.0)
    {
        flDamage -= count;
        num++;
    }
